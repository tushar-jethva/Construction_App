...
class AddBillingPartyBloc
  ...
  AddBillingPartyBloc(
      {required this.projectRepository, required this.billingPartyRepository})
      : super(AddBillingPartyState()) {
    on<LoadProjectsBillingEvent>((event, emit) async {
      try {
        emit(state.copyWith(
            isLoadingProject: false, projectValue: state.projectValue));
    ...
    on<AddBillingParty>((event, emit) async {
      try {
        ...
        await billingPartyRepository.addBillingParty(
    ...
    on<Initialize>((event, emit) {
      emit(state.copyWith(
          projects: [], isAdded: 0, projectValue: "", isLoadingProject: false));
    });
  }
}
