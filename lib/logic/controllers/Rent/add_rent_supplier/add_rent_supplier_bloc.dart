...
part 'add_rent_supplier_event.dart';
part 'add_rent_supplier_state.dart';
part 'add_rent_supplier_bloc.freezed.dart';

@singleton
class AddRentSupplierBloc
    extends Bloc<AddRentSupplierEvent, AddRentSupplierState> {
  AddRentSupplierBloc(this.agencyRepository)
      : super(AddRentSupplierState.initial()) {
    on<AddRentSupplierEvent>((event, emit) async {
      await event.map(
        started: (_Started value) {
          emit(AddRentSupplierState.initial());
        },
        ...
        addRentSupplier: (_AddRentSupplier value) async {
